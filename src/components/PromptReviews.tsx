...
export function PromptReviews({ promptId }: { promptId: string }) {
  ...
  async function submitReview(e: FormEvent) {
    ...
    setSubmitting(true);
    const { error } = await supabase.from("prompt_reviews").upsert(
      {
        prompt_id: promptId,
        user_id: user.id,
        rating,
  ...
  async function deleteReview(id: string) {
    const { error } = await supabase.from("prompt_reviews").delete().eq("id", id);
